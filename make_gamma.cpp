...
main(int argc, char **argv) {
  ...
  double pagivend, pbgivend;
  double sum;

  for(int dn=0; dn < data.size(); dn++) {
      pagivend = chce_probs[0] * pow(ht_probs[0][0], data[dn].ht_cnts[0]) * pow(ht_probs[0][1], data[dn].ht_cnts[1]);
      pbgivend = chce_probs[1] * pow(ht_probs[1][0], data[dn].ht_cnts[0]) * pow(ht_probs[1][1], data[dn].ht_cnts[1]);
      sum = pagivend + pbgivend;
      gamma[dn][0] = pagivend/sum;
      gamma[dn][1] = pbgivend/sum; 
  }
